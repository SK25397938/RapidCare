export default function Topbar({ surgeMode, onToggleSurgeMode, connectionState, totals }) {
  return (
    <header className="relative z-20 flex flex-wrap items-center justify-between gap-4 px-4 pt-4 lg:px-5">
      <div className="flex items-center gap-4 rounded-[26px] border border-white/10 bg-white/[0.06] px-5 py-4 backdrop-blur-2xl">
        ...
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.38em] text-slate-400">Healthcare Emergency Platform</p>
          <h1 className="mt-1 font-['Space_Grotesk'] text-3xl font-semibold text-white">RapidCare</h1>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-[26px] border border-white/10 bg-white/[0.06] px-4 py-3 backdrop-blur-2xl">
        <div className="flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-4 py-2 text-sm text-emerald-100">
          ...
          {connectionState === "live" ? "Live Mode" : connectionState}
        </div>
        ...
        >
          ...
          <span
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${
              surgeMode ? "bg-rose-500/90" : "bg-slate-700"
            }`}
          >
            <span
              className={`h-4 w-4 rounded-full bg-white transition ${
                surgeMode ? "translate-x-6" : "translate-x-1"
              }`}
            />
          </span>
        </button>
        ...
        <div className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-slate-200">
          {totals.available} green | {totals.limited} orange | {totals.full} red
        </div>
      </div>
    </header>
  );
